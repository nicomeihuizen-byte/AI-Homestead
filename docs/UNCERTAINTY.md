...
|---|---|---|---|---|---|
| `air_temp_c` | BME280 in ASA shield | +/-1.0 C | | self-heating if not forced mode; shield error up to ~3 C in sun and low wind | De Bilt 260, 14 d |
| `air_rh_pct` | BME280 | +/-3% | | | |
| `air_pressure_hpa` | BME280 | +/-1 hPa | | | |
| `soil_temp_*_c` | DS18B20 | +/-0.5 C | | none worth arguing about | ice water |
| `soil_vwc_*` | capacitive | — | | strong temperature cross-sensitivity; ink mask degrades within a year; false-dry as soil shrinks away | oven-dried jars, per probe |
| `rain_mm` | tipping bucket | 0.2794 mm/tip | | reed bounce inflates totals 10-30% if undebounced; under-reads if not level | De Bilt monthly totals |
| `wind_kmh` | 3-cup reed | 2.4 km/h per Hz | | stalls below ~1-1.5 m/s: calm reads exactly 0 | |
| `wind_dir` | 8-reed vane | 22.5 deg | | | |
| `lux` | BH1750 | — | | photopic-weighted. **NOT PAR** — deriving PPFD is +/-15% at best vs +/-5% for a quantum sensor, and the coefficient is only valid for the spectrum you calibrated under | |
